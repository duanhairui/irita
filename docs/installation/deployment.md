...
for i in `seq 0 $[ ${#DockerIP[*]} -1 ]`; do docker -H ${DockerIP[$i]} run -itd -e ChainCMD=$ChainCMD -e NodeName=${Names[$i]} -e PointToken="$PointToken" -v $DataPath/$NodeName-$i:/root --name $NodeName-$i bianjie/irita; done
