...
// createAppAndExport creates a new IRITA app (optionally at a given height)
// and exports state.
func createAppAndExport(
	...
	if height != -1 {
		iritaApp = app.NewIritaApp(logger, db, traceStore, false, map[int64]bool{}, "", uint(1), encCfg)
		if err := iritaApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		iritaApp = app.NewIritaApp(logger, db, traceStore, true, map[int64]bool{}, "", uint(1), encCfg)
	}
